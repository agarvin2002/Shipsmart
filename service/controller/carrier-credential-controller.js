  ...
  static async getCredentialById(req, res, next) {
    try {
      ...
      if (credential.error) {
        ...
        return res.status(404).send(ResponseFormatter.formatError(credential.error, req.id, 404));
      }
  ...
  static async createCredential(req, res, next) {
    try {
      ...
      if (credential.error) {
        ...
        return res.status(400).send(ResponseFormatter.formatError(credential.error, req.id, 400));
      }
