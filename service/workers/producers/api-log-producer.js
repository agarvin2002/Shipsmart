 ...
 * ApiLogProducer
 *
 * Queues API request/response logs for async database storage.
 * Lower priority than critical jobs (rate fetch, etc.)
 */
class ApiLogProducer {
  ...
  static getProducer() {
    const queue = workerClient.getQueue(WorkerJobs.API_LOG);
    const jobOpt = {
      attempts: 2,
    ...
    return new ApiLogProducer(queue, jobOpt);
  }
}

module.exports = ApiLogProducer;
